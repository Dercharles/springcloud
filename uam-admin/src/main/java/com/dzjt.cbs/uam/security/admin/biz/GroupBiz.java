...
import com.dzjt.cbs.uam.security.admin.mapper.GroupMapper;
...
public class GroupBiz extends BaseBiz<GroupMapper, Group> {
    ...
    public void modifyAuthorityMenu(int groupId, String[] menus,String type) {
        if(!AdminCommonConstant.RESOURCE_TYPE_MENU.equals(type) && !AdminCommonConstant.RESOURCE_TYPE_BTN.equals(type)){
            throw new BaseException("type illegal:"+type);
        }
        resourceAuthorityMapper.deleteByAuthorityIdAndResourceType(groupId + "", type);
...
//        if(type.equals(AdminCommonConstant.RESOURCE_TYPE_MENU)) {
//            for (String menuId : menus) {
//                findParentID(map, relationMenus, menuId);
//            }
//        }
        for (String menuId : relationMenus) {
            authority = new ResourceAuthority(AdminCommonConstant.AUTHORITY_TYPE_GROUP, type);
...
//    }
     ...
     * @return
     */
    public List<Integer> getAuthorityElement(int groupId,String type) {
        ResourceAuthority authority = new ResourceAuthority(AdminCommonConstant.AUTHORITY_TYPE_GROUP, type);
        authority.setAuthorityId(groupId + "");
    ...
    public List<Group> getGroupsByUserId(int userId){
        return groupMapper.selectGroupByMemberId(userId);
    }
     ...
     * @return
     */
    public boolean modifyGroupsMemberByUser(int userId,List<Integer> groupIdList){
        mapper.deleteGroupMembersByUserId(userId);
        groupIdList.stream().forEach(id->mapper.insertGroupMembersById(id,userId));
        return true;
    }
}
